...
class ElbowFlex(gymnasium.Env):
   ...
   #}
    def __init__(self, episode_limit=1000, seed=0):
        ...
        """
        self.MyoEnv = gym.make(env_name,normalize_act=False,muscle_condition="sarcopenia",joint_random_range=(0, 0))
        self.agent1=PPO.load("MyoAssistAgent1")
        ...
        # Define action space
        self.action_space = spaces.Box(low=self.ACT_RANGE[0], high=self.ACT_RANGE[1], shape=(1,), dtype=np.float32)
    ...
    def step(self, action):  
        observation=self.get_obs_vec(self.get_obs_dict(np.zeros(self.action_space.shape[0]))) 
        mus_action,_ = self.agent1.predict(observation, deterministic=True)
        mus_action = (mus_action+1)/2
        exo_action = np.append(action[0], mus_action) # exo action is zero for hlthy
        self.MyoEnv.step(exo_action)
        self.obs_dict=self.get_obs_dict(exo_action)
