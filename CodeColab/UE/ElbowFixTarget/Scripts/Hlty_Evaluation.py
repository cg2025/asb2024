...
def evaluate(env_name, train_steps, run_id, weight_value, target_value):
    env_name_2 = 'elbow-v0'
    ...
    for num_ep in range(num_eps):
        ...
        for timestep in range(100):      
            ...
            obs = np.concatenate((state[:2],error,state[3:]),axis=None)
...
def get_frames(env_name, train_steps, run_id, weight_value, target_value):
    env_name_2 = 'elbow-v0'
    p = utils.init_parameters(train_steps = train_steps)
    env_name_2 = utils.get_env_prefix(False, False)+p['new_model_nm']
    env = gym.make(env_name, normalize_act=False,reset_type='init')
    ...
    for t in target:
        ...
        state[0]= env.env.sim.data.qpos[0]
        state[1]= env.env.sim.data.qvel[0]
        obs= np.concatenate((state[:2],state[0]-t,state[3:]),axis=None)
...
vid_frames= get_frames(env_name, train_steps, run_id, weight_value, target_value)
frames2Video('CodeColab/UE/ElbowFixTarget/vid_plots/healthy.mp4', vid_frames)
